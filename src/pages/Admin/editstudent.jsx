...
// Subject mappings (same as backend)
...
const EditStudentForm = () => {
 ...
 '1 year'];
  ...
  const certifications = Object.keys(certificationSubjectMap);
  ...
  const handleSearch = async (data) => {
    try {
      setLoading(true);
      const searchParams = {};
      if (data.phoneNumber) searchParams.phoneNumber = data.phoneNumber;
      if (data.rollNo) searchParams.rollNo = data.rollNo;

      if (!searchParams.phoneNumber && !searchParams.rollNo) {
        toast.error('Please provide either phone number or roll number');
        return;
      }

      const response = await axios.put('/api/students/edit', searchParams);
      ...
      if (response.data.data) {
        ...
        Object.keys(studentData).forEach((key) => {
          if (key === 'dateOfBirth' || key === 'joiningDate') {
            ...
            setInstallments(studentData.feeDetails?.installmentDetails.map(inst => ({
              ...inst,
              submissionDate: new Date(inst.submissionDate),
            })) || []);
          } else if (key === 'examResults') {
            setValue('examResults', studentData.examResults.map(result => ({
              ...result,
  ...
  const handleFormSubmit = async (data) => {
    try {
      ...
      const response = await axios.put('/api/students/edit', payload);
              ...
              {activeStep === 0 && (
                <div>
                  <Grid container spacing={3}>
                    <Grid item xs={12}>
                      <FormControl fullWidth>
                        ...
                        <Select
                          ...
                          label="Search By"
                        >
                          <MenuItem value="both">Phone Number or Roll Number</MenuItem>
                          <MenuItem value="phone">Phone Number Only</MenuItem>
                          <MenuItem value="roll">Roll Number Only</MenuItem>
                        </Select>
                      </FormControl>
                    </Grid>

                    {(searchMode === 'both' || searchMode === 'phone') && (
                      <Grid item xs={12}>
                        <TextField
                          fullWidth
                          label="Phone Number"
                    ...
                    {(searchMode === 'both' || searchMode === 'roll') && (
                      <Grid item xs={12}>
                        <TextField
                          fullWidth
                          label="Roll Number"
                ...
                <div>
                  ...
                  {activeStep === 2 && (
                    <Grid container spacing={3}>
                      ...
                      <Grid item xs={12} md={6}>
                        <FormControl fullWidth>
                          <InputLabel>Certification Title</InputLabel>
                          <Select
                            label="Certification Title"
                            {...register('certificationTitle', { required: 'Required field' })}
